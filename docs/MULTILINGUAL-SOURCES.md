...
This document records the source-registry foundation. All registry entries remain disabled by default. The approved Amharic source has a verified storage foundation documented in `docs/TRANSLATION-PACKS.md` and becomes selectable only inside the Ayah Context Lens after a complete pack is installed. The existing Saheeh International translation continues through the legacy singular resource in `app/content-manifest.ts`; Ibn Kathir resource 169 is unchanged.
...
| --- | --- | --- | --- | --- | --- | --- |
| `quranenc:amharic_zain` | Amharic (`am`, `amh`), Ethiopic, LTR | Muhammad Zain Zahruddin; Africa Academy; publisher: Africa Academy | Approved candidate; registry disabled; available in the Ayah Context Lens only after verified pack activation | `1.0.1`; `1.0.1-xml.1`; published 2024-06-11; updated 2026-01-20 | 114 surahs, 6,236 ayat; no missing, duplicate, empty, invalid, or wrong-script records | QuranEnc permits download and republication under its conditions; explicit verified installation is supported by `app/translation-packs.mjs` |

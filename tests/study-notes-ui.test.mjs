...
test("note save revalidation uses the trusted loaded-page cache before the network fallback", async () => {
  ...
  const source = page.slice(start, end);
  assert.match(source, /pageCacheRef\.current\.get\(quranPageCacheKey\(activeReadingId, capturedAnchor\.page\)\)/);
  assert.match(source, /isVerifiedPage\(cachedPage, capturedAnchor\.page, activeReadingId\)/);

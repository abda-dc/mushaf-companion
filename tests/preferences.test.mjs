...
test("migrates older preference documents into navigation history and download settings", () => {
  const storage = memoryStorage({
    "mushaf:preferences-v3": JSON.stringify({ version: 3, reader: { lastPage: 18, lastVerse: "2:142", lastVersePage: 22 }, bookmarks: ["22|2:142"] }),
  });
  const preferences = loadPreferences(storage);
  assert.equal(preferences.version, 4);
  ...
  assert.equal(preferences.downloads.wifiOnly, true);
  assert.ok(storage.values.has("mushaf:preferences-v4"));
});
...
test("portable backups normalize restored progress and reader settings", () => {
  const storage = memoryStorage();
  const saved = savePreferences(storage, { reader: { lastPage: 604, lastVerse: "114:6", lastVersePage: 604, recentPages: [604, 582, 562], theme: "dark", translation: true }, bookmarks: ["604|114:6"] });

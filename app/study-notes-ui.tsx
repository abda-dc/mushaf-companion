...
function NoteCard({ note, onEdit, onDelete }: { note: StudyNote; onEdit: (note: StudyNote, trigger: HTMLButtonElement) => void; onDelete: (id: string) => void }) {
  ...
  return <article className="private-note-card" ref={articleRef} data-note-id={note.id}>
    <header><div><span>PRIVATE NOTE · USER-AUTHORED</span><strong>{note.anchor.type === "word" ? `Word ${note.anchor.wordPosition} · Ayah ${note.anchor.verseKey}` : `Ayah ${note.anchor.verseKey}`}</strong></div><small>Edited {formatEdited(note.updatedAt)}</small></header>
...
function IndexNoteCard({ note, deleteArmed, onArmDelete, onCancelDelete, onOpen, onDelete, fallbackFocus }: {
  ...
  return <article ref={articleRef} data-note-id={note.id}><button type="button" className="note-open" onClick={(event: MouseEvent<HTMLButtonElement>) => onOpen(note, event.currentTarget)}><span>{note.anchor.type === "word" ? "PRIVATE WORD NOTE" : "PRIVATE AYAH NOTE"}</span><strong>Ayah {note.anchor.verseKey}{note.anchor.type === "word" ? ` · word ${note.anchor.wordPosition}` : ""}</strong><p dir="auto">{note.body}</p><small>Page {note.anchor.page} · edited {formatEdited(note.updatedAt)}</small></button><div>{deleteArmed ? <><button ref={confirmDeleteRef} type="button" className="confirm-delete" onClick={() => { const target = adjacentNoteAction(articleRef.current); onDelete(note.id); scheduleFocus([target, fallbackFocus]); }}>Confirm delete</button><button type="button" onClick={() => { onCancelDelete(); scheduleFocus([() => deleteTriggerRef.current]); }}>Cancel</button></> : <button ref={deleteTriggerRef} type="button" onClick={() => onArmDelete(note.id)} aria-label={`Delete private note for ayah ${note.anchor.verseKey}`}>Delete</button>}</div></article>;
}

export function StudyNotesIndex({ notes, onOpen, onDelete, onRenameTag, onRemoveTag }: StudyNotesIndexProps) {
  const [query, setQuery] = useState("");
  const [anchorType, setAnchorType] = useState<"all" | "ayah" | "word">("all");
  ...
  const filtered = useMemo(() => searchStudyNotes({ schemaVersion: 1, notes }, query, anchorType, tag), [notes, query, anchorType, tag]);
  ...
  return <div className="notes-index">
    ...
    <div className="notes-index-tools"><label><span className="sr-only">Search private notes</span><input ref={searchInputRef} value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search note text, tag, or verse" dir="auto" /></label><label><span className="sr-only">Filter note anchor</span><select value={anchorType} onChange={(event) => setAnchorType(event.target.value as "all" | "ayah" | "word")}><option value="all">All notes</option><option value="ayah">Ayah notes</option><option value="word">Word notes</option></select></label></div>
